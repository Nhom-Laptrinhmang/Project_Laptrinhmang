...
namespace ChatApp.Client.Forms
{
    public partial class ChatForm : Form
    {
        ...
        private void ClientService_OnPacketReceived(MessagePacket packet)
        {
            ...
            {
                string displayContent = packet.Content.Trim();
                string senderName = !string.IsNullOrEmpty(packet.Sender) ? packet.Sender.Trim() : "";

                if (packet.Type == Protocol.Connect || packet.Type == Protocol.Disconnect)
                {
                    UpdateUserListUI(displayContent);
                }
                ...
                bool isSystemMsg = (senderName == "Hệ thống" || packet.Type == Protocol.Connect || packet.Type == Protocol.Disconnect);
        ...
        private void UpdateUserListUI(string userListContent)
        {
            ...
            foreach (string user in onlineUsers)
            {
                ...
                if (trimmedUser != _myUsername) lstUsers.Items.Add($"{trimmedUser} (● Đang hoạt động)");
            }
        ...
        private void AddMessengerBubble(string sender, string message, bool isMe, bool isSystem = false, Image avatar = null, string messageId = null)
        {
            ...
            Panel bubbleContainer = new Panel
            {
                ...
                Margin = new Padding(0, 5, 0, 15), // Kéo dãn lề dưới để không bị lấp
                BackColor = Color.Transparent
            };

            // 1. TẠO VÀ CĂN GIỮA THỜI GIAN
            ...
            // 2. MỐC TỌA ĐỘ MỚI (Đẩy tất cả phần tử khác xuống dưới chữ thời gian)
            ...
            ToolStripMenuItem mnuForward = new ToolStripMenuItem("Chuyển tiếp (Forward)");
            mnuForward.Click += (s, ev) =>
            {
                var forwardPacket = new MessagePacket
                {
                    Type = Protocol.Message,
                    Sender = _myUsername,
                    Content = $"[Chuyển tiếp từ {sender}]: {message}",
                    Receiver = "All",
                    AvatarBase64 = _myAvatarBase64,
                    MessageId = Guid.NewGuid().ToString()
                };
                _clientService.SendPacket(forwardPacket);
                AddMessengerBubble("Hệ thống", "Đã chuyển tiếp tin nhắn thành công.", isMe: false, isSystem: true);
            };
            msgMenu.Items.Add(mnuForward);
            lblMessage.ContextMenuStrip = msgMenu;

            // 3. VẼ BONG BÓNG THEO TỌA ĐỘ MỚI (startY)
            if (isSystem)
            {
            ...
            // Xử lý thanh cuộn (Tự kéo Max xuống đáy)
        ...
        private void ClientService_OnDisconnected()
        {
            ...
            AddMessengerBubble("Hệ thống", "Mất kết nối mạng an toàn tới máy chủ Server.", isMe: false, isSystem: true);
        }
